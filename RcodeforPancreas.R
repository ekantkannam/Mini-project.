...
setwd('C:/Users/OneDrive/Documents/ngs/Pancreas')
# Read count RNA-Seq data
count_data <- read_csv('GSE164416_raw_counts_GRCh38.p13_NCBI (1).csv')
...
Type <- rep(c("Pancreatic tissue Healthy", "Pancreatic tissue T2D"), each = ncol(counts_data)/2)  # Adjusted condition labels
...
write.csv(res_df, "Pancreatic_results.csv", row.names = TRUE)
cat("DESeq2 results saved to 'Pancreatic_results.csv'\n")
write.csv(res_df, file = "Pancreatic_results.csv", row.names = TRUE)
...
sample_annotations <- data.frame(
  Condition = rep(c("Pancreatic tissue Control", "Pancreatic tissue T2D"), length.out = nrow(sig_genes_data_transposed))
)
...
sample_annotations <- data.frame(
  Condition= rep(c("Pancreatic tissue Control", "Pancreatic tissue T2D"), length.out = length(colnames(sig_genes_data)))
)
rownames(sample_annotations) <- colnames(sig_genes_data)
sample_annotations <- data.frame(
  Condition = rep(c("Pancreatic tissue Control", "Pancreatic tissue T2D"), length.out = length(colnames(sig_genes_data)))
...
head(res_df)
write.csv(res_df, "Pancreatic_annotations.csv", row.names = TRUE)
